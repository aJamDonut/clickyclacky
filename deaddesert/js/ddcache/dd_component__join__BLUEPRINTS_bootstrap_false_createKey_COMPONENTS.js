(function () {
  ...
  _BLUEPRINTS.COMPONENTS.hud_voice_actor = {
    "getElement": function (options) {
      if (!options) {
        options = {};
      }
      ...
      dummy.scale.set(1.5);
      dummy.setAnim("util/bandage", true);
      game.render.spine.hackSpineSlot("melee", dummy.spine, game.render.draw.generateTexture(new Sprite('weapon_magic_wand')));
      game.render.spine.hackSpineSlot("head_down", dummy.spine, game.render.draw.generateTexture(new Sprite('blank')));
      game.render.spine.hackSpineSlot("head_right", dummy.spine, game.render.draw.generateTexture(new Sprite('blank')));
      let counter = 0;
      let anim = "util/bandage";
      let ticks = 0;
      let countTicks = true;
      container.onTick(() => {
        if (!countTicks) return;
        counter++;
        if (counter < 24) return;
        ticks++;
        if (ticks > 10) {
        ...
        const textr = game.render.draw.generateTexture(spriteFromTexture, {
          region: clippingArea
        });
        game.render.spine.hackSpineSlot("head_mask", dummy.spine, textr);
        game.render.spine.hackSpineSlot("head_mask_right", dummy.spine, textr);
        spriteFromTexture.destroy();
      });
      container.addChild(dummy);
      let currentBodyIndex = 0;
      const updateBody = () => {
        const body = bodies[currentBodyIndex];
        game.render.spine.hackSpineSlot("body_down", dummy.spine, game.render.draw.generateTexture(new Sprite(body)));
        game.render.spine.hackSpineSlot("body_right", dummy.spine, game.render.draw.generateTexture(new Sprite(body + "right")));
      };
      const leftButton = game.render.component('hud_button', {
        name: '<',
        w: 50,
      ...
      container.addChild(rightButton);
      updateBody();
      anims.forEach((animObj, index) => {
        const animButton = game.render.component('hud_button', {
          name: animObj.name,
          w: 25,
          h: 25
        });
        animButton.scale.set(0.5);
        animButton.x = 10;
        animButton.y = 10 + index * (animButton.height + 5);
        animButton.keep = true;
        animButton.on('pointerup', () => {
          if (!animObj.dir) {
            dummy.setDir("down");
            dummy.tick();
          }
          if (animObj.dir) {
            dummy.setDir(animObj.dir);
            dummy.tick();
          }
          if (animObj.anim) {
            anim = animObj.anim;
      ...
      container.addChild(saveButton);
      const mouth = game.render.componentRaw('composite_mouth_animator');
      dummy.addChild(mouth);
      mouth.scale.set(1.7);
      mouth.interactive = true;
      mouth.buttonMode = true;
      mouth.on('pointerdown', onDragStart).on('pointerup', onDragEnd).on('pointerupoutside', onDragEnd).on('pointermove', onDragMove);
  ...
  _BLUEPRINTS.COMPONENTS.composite_mouth_animator = {
    "options": '\'loadFile, type=new|load, slot\'',
    "getElement": function (options) {
      if (!options) {
        options = {};
      ...
      game.render.lifeLayer.addChild(mouth);
      mouth.scale.set(0.4);
      mouth.x = 0;
      mouth.y = 0;
      mouth.play();
      white.addChild(mouth);
      const startBtn = game.render.component('hud_button', {
        name: 'Start',
        w: 100,
        h: 50
      });
      startBtn.x = 10;
      startBtn.y = 10;
      pane.addChild(startBtn);
      const stopBtn = game.render.component('hud_button', {
        name: 'Stop',
        w: 100,
        h: 50
      });
      stopBtn.x = 120;
      stopBtn.y = 10;
      pane.addChild(stopBtn);
      const status = game.render.text('Status: Idle', 'ingame-label');
      status.x = 10;
      status.y = 70;
      pane.addChild(status);
      const inputDeviceSelect = game.render.component('hud_dropdown', {
        w: 200,
        h: 40,
        items: [],
        selected: 'Select Device'
      });
      inputDeviceSelect.x = 10;
      inputDeviceSelect.y = 120;
      pane.addChild(inputDeviceSelect);
      let audioContext;
      let microphone;
      let analyser;
      let dataArray;
      ...
      function monitorAudio() {
        ...
        status.text = `Audio input level: ${decibels.toFixed(2)} dB`;
        if (decibels > dblow) {
          mouth.play();
        }
        requestAnimationFrame(monitorAudio);
      }
      function monitorRemoteAudio(remoteStream) {
        ...
        function updateAudioLevel() {
          ...
          const rms = Math.sqrt(sum / dataArray.length);
          const decibels = 20 * Math.log10(rms);
          status.text = `Remote Audio Level: ${decibels.toFixed(2)} dB`;
          if (decibels > dblow) {
            mouth.play();
          }
      ...
      startBtn.on('pointerup', startListening);
      stopBtn.on('pointerup', stopListening);
      pane.enableEvents();
      pane.on('rightclick', function () {
      ...
      if (usePeer) {
        ...
        navigator.mediaDevices.getUserMedia({
          ...
          peer.on('open', id => {
            game.peerid = id;
            alert(`Your peer ID is: ${id}. Share this ID with the other peer.`);
          });
        });
      }
      return holder;
